...
import { GradeProImportDialog } from './GradeProImportDialog';
...
describe('GradeProImportDialog', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('is hidden when isOpen is false', () => {
    render(<GradeProImportDialog {...defaultProps} isOpen={false} />);
    expect(screen.queryByText('Import GradePro')).toBeNull();
  });

  it('renders file input when dialog is open', () => {
    render(<GradeProImportDialog {...defaultProps} />);
    expect(screen.getByText('Import GradePro')).toBeDefined();
  ...
  it('calls onClose when Cancel is clicked in preview step', async () => {
    const onClose = vi.fn();
    render(<GradeProImportDialog {...defaultProps} onClose={onClose} />);
  ...
  it('shows success summary after a successful import', async () => {
    const successResult = {
      created: { picos: 2, outcomes: 5, recommendations: 3 },
      skipped: 1,
    };
    postMock.mockResolvedValue({ data: successResult });

    const onSuccess = vi.fn();
    render(<GradeProImportDialog {...defaultProps} onSuccess={onSuccess} />);
